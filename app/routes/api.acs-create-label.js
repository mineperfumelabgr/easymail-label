...
const requestedContentTypeId = requestedContentTypeIdRaw
  ...
  : 7;
    ...
    if (!process.env.ACS_BILLING_CODE) {
      return cors(jsonFAIL("Missing ACS_BILLING_CODE in environment.", 200));
    }
    ...
    if (!zip) {
      return cors(jsonFAIL("Missing shipping ZIP / postal code.", 200));
    }
    if (!zipDigits) {
      return cors(jsonFAIL(`Invalid shipping ZIP / postal code: ${zip}`, 200));
    }

    const region = safeStr(ship.province) || safeStr(ship.city);
    if (!region) {
      return cors(jsonFAIL("Missing shipping city/region.", 200));
    }
    ...
    if (!street) {
      return cors(jsonFAIL("Missing shipping address line.", 200));
    }
    ...
    if (!masterVoucherNumber) {
      return cors(jsonFAIL("ACS did not return Voucher_No.", 200));
    }
